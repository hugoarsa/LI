...
% USAR 'swipl' para interprete

% nom de fichero en minuscula se carga con '[nombreFichero].' y devuelve true
% luego podemos hacer queries del tipo 'padre(X,pedro)' y 'padre(pedro,X)'

% si elegimos la primera opcion 'padre(X,pedro)' se queda abierto a ver si se cumple
% o no y hasta que no le demos a ; no hace backtracking

% ahora bien si usamos 'padre(X, pedro), write(X), nl' y asi escribe cuando encuentra
% si quieres que haga backtraking automatico añadimos fail al final (muy practico)
...
% | ?- f(X,a) = f(b,Y).
% | ?- f(f(X),a)  =  f(Y,Y).  estos no son unificables porque requeriria de que la Z fuese a la vez f(X) y a
% | ?- f(f(X),a) \=  f(Y,Y).  si pregunto si no son unificables me confirma que no lo son
% | ?- f(f(X),a)  =  f(Y,Z).

% | ?- f(g(X,T),Y)  =  f(g(b,Z),Z).   aixo dona com a resposta T = Z, Y = Z
...
long([],0).
long([_|L],N):- long(L,N1), N is N1+1. %el is necesita que el predecesor ya este instanciado de alguna forma (por eso N1 se hace antes)
