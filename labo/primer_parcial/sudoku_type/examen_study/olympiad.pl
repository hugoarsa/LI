...
compatibleStudents(S1, S2):- student(S1), student(S2), friends(S1, S2), friends(S2, S1).
topic(E):- info(_,_,E).
...
allDifferentTopics:-
      topic(E),
      findall(chosen(S), expertise(S, E), Lits),
      atMost(1, Lits),
      fail.
allDifferentTopics.
...
allFriends:-
      student(S1),
      student(S2),
      not(compatibleStudents(S1, S2)),
      writeOneClause([-chosen(S2), -chosen(S1)]),
      fail.
allFriends.
