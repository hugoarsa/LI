...
satVariable( threeNights(Ph,N) ):- pharmacy(Ph), night(N).  % means "pharmacy Ph is on duty three nights from night N"
...
minimunOnDutyPharmaciesEachNight:-
      ...
      findall(phn(Ph,N),pharmacy(Ph),Lits),
      atLeast(MINPh, Lits), fail.
minimunOnDutyPharmaciesEachNight.

% b) each pharmacy has a list of nights when it cannot be on duty
blockedNights:- 
      pharmacy(Ph),
      findall(phn(Ph,N),(night(N),pharmacyBlockedNight(Ph,N)),Lits),
      exactly(0, Lits), 
      fail.
blockedNights.
...
noThreeConsecutiveNights:- 
      pharmacy(Ph),
      findall(threeNights(Ph,N),night(N),Lits),
      exactly(0, Lits), 
      fail.
noThreeConsecutiveNights.
...
threeRestriction:- 
      pharmacy(Ph), 
      threeConsecutiveNights(N1,N2,N3),
      %(phn(Ph,N1) & phn(Ph,N2) & phn(Ph,N3)) -> threeNights(Ph,N1)
      writeOneClause([-phn(Ph,N1),-phn(Ph,N2),-phn(Ph,N3),threeNights(Ph,N1)]),
      fail.
threeRestriction.
...
nearbyOnDutyPharmacies:- 
      night(N),
      pharmacy(Ph1),
      nearbyPharmacies(Ph1,LPh),
      findall(phn(Ph2,N),member(Ph2,LPh),Lits),
      atLeast(1,[phn(Ph1,N)|Lits]),
      fail.
nearbyOnDutyPharmacies.
