...
matrixByRows(S, L , [L]) :- length(L, S).
matrixByRows(S, L, [L1|MatrixByRows]) :- append(L1, L2, L), length(L1, S), matrixByRows(S, L2, MatrixByRows).

constraintsFromSubLists([]).
constraintsFromSubLists([Elem|MatrixByX]) :- all_distinct(Elem), constraintsFromSubLists(MatrixByX).
