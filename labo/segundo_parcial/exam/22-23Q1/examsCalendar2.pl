...
%% Exams should take [X,Y] place within the specified dates. In each day
...
definitionExamOnDay :- subject(S), day(D), findall(x(S,D,I), slot(I), Lits), expressOr(x(S,D), Lits), fail.
definitionExamOnDay.

nobodyWithThreeExamsOnSameDay :-
    subject(S1), subject(S2), subject(S3),
    S1 \= S2, S2 \= S3, S1 \= S3,
    shareSomeStudent(S1, S2, S3), day(D),
    writeClause([-x(S1,D), -x(S2, D), -x(S3, D)]), fail.
nobodyWithThreeExamsOnSameDay.
...
costOfThisSolution(M, Cost):-
    findall( (U,D), (studentSubject(U,S1), studentSubject(U,S2), S1\=S2,
                 member(x(S1,D,_),M), member(x(S2,D, _),M)), L ), sort(L,L1), length(L1,Cost), !.


% The following definition is not accurate because, by the definition of definitionTwoExamsOnSameDay, t(U, D) can be true even if the student U does not have 2 exams on day D
% costOfThisSolution(M, Cost):- findall((U,D), member(t(U,D),M),L), sort(L1, L1), length(L1, Cost), !.
