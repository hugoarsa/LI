...
eachCourseExactlyOneProf:- course(C), courseProfessors(C,LP), findall( cp(C,P), member(P,LP), Lits ), exactly(1,Lits), fail.
eachCourseExactlyOneProf.

eachCourseExactlyOneRoom:- course(C), courseRooms(C,LR), findall( cr(C,R), member(R,LR), Lits ), exactly(1,Lits), fail.
eachCourseExactlyOneRoom.

eachCourseMaxOncePerDay:-  course(C), day(D), findall( cdh(C,D,H),hour(H), Lits ), atMost(1,Lits), fail.
eachCourseMaxOncePerDay.

% cd <-> cdh V cdh V cdh  (que haya un curso en un dia implica que tiene que alguna de sus horas es curso [ya definimos antes que solo una])
relateVarsCDHwithCD:- course(C), day(D), findall( cdh(C,D,H),hour(H), Lits ), expressOr(cd(C,D),Lits), fail.
relateVarsCDHwithCD.

%aqui podemos operar con cd en vez de con cdh para reducir el numero de clausas pues estan ligadas
%por tanto miramos que haya tantos dias por curso como se estipula en su definicion
eachCourseRightNumberOfDays:- course(C), courseHours(C,H), findall(cd(C,D),day(D),Lits), exactly(H,Lits), fail.
eachCourseRightNumberOfDays.

noOverlapCoursesSameYear:- year(Y), courseYear(C1,Y), courseYear(C2,Y), C1 < C2, hour(H), day(D), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H) ]), fail.
noOverlapCoursesSameYear.


noOverlapProfs:- course(C1), course(C2), C1 < C2, courseProfessors(C1,LP1), courseProfessors(C2,LP2),
    professor(P), member(P, LP1), member(P, LP2), hour(H), day(D), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.


noOverlapRooms:- course(C1), course(C2), C1 < C2, courseRooms(C1,LR1), courseRooms(C2,LR2),
    room(R), member(R, LR1), member(R, LR2), hour(H), day(D), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R) ]), fail.
noOverlapRooms.

%usedProf <-> cp v cp v cp
relateVarsCPwithUsedProf:- professor(P), findall( cp(C,P), course(C), Lits ), expressOr(usedProf(P), Lits), fail.
relateVarsCPwithUsedProf.

%no funca
maxProfs(MaxNumProf):- numProfessors(N), N > MaxNumProf, findall( usedProf(P), professor(P), Lits ), atMost(MaxNumProf, Lits), fail.
maxProfs(_).
