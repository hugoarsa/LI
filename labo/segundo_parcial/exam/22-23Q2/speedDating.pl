...
%si hay dos citas las cuales comparten algun participante (confio en que esa definicion esta bien, para todos los slots posibles estas dos citas no podran suceder a la vez
...
% ps <-> pps V pps V pps  (si hay una cita donde p es uno de los dos integrantes ps es cierta)
relatePSwithPPS(MaxSlots):- participant(P), slot(S), between(1,MaxSlots,S), findall( pps(P-P1,S),date(P-P1), Lits1 ), findall( pps(P1-P,S),date(P1-P), Lits2 ), append(Lits1, Lits2,Lits), expressOr(ps(P,S),Lits), fail.
relatePSwithPPS(_).

%cada k citas consecutivas la persona p que tenga k citas consecutivas ha de descansar
% ps1 & ... & psk -> -(psk+1)
%esta peta y no se porque...
restEveryKConsecutiveDates(MaxSlots):- maxConsecutiveDates(K), K1 is K+1, participant(P), kConsecutiveSlots(MaxSlots,K,LKS), sort(LKS,L1), last(L1,Temp), MAX = Temp + 1, findall(ps(P,S), member(S,LKS), Lits), findall(NLit, (member(Lit,Lits), negate(Lit,NLit)), NLits), negate(Var,ps(P,MAX)), writeOneClause([ Var | NLits]), fail.
restEveryKConsecutiveDates(_).

last([X],X).
last([_|L],N):- last(L,N).

%el maximo de citas en cada slot viene naturalmente definido por el numero de mesas de las que disponemos
maxDatesAtTheSameSlot(MaxSlots):- numTables(T), slot(S), between(1,MaxSlots,S),findall(pps(P2-P1,S),date(P1-P2), Lits), atMost(T, Lits), fail.
maxDatesAtTheSameSlot(_).

% (cita de plebeyos en s - 1) -> (cita de vip en s - 1)
betweenVipsBefore(MaxSlots):- slot(S), between(2,MaxSlots,S), Sprev is S - 1, vips(LV), date(P1,P2), member(P1,LV), member(P2,LV), date(P3,P4), \+member(P3,LV), \+member(P3,LV), writeOneClause([ -pps(P3-P4,Sprev), -pps(P1-P2,S) ]), fail.
betweenVipsBefore(_).
...
%aqui teniendo la solucion en M he de determinar su coste, este sera el numero de slots usados
%usamos el sort para eliminar repetidos
costOfThisSolution(M,Cost):- findall(S,member(ps(_,S),M),L), sort(L,L1), length(L1,Cost),!.
