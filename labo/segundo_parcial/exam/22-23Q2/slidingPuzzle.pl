...
step(OldBoard, NewBoard) :-
    ...
    length(OldRow, Len),
    between(1,Len,Pos),                  %% cojo el resto de la row que me queda y
    nth1(Pos, NewRow, x, RestRow),       %% pongo la x en un punto arbitrario entre estos
...
oneStep(1, OldBoard, NewBoard) :- %%un movimiento por filas
    step(OldBoard, NewBoard).
oneStep(1, OldBoard, NewBoard) :- %%un movimiento por columnas es un movimiento por filas traspuesto
    transpose(OldBoard,OldBoardT),
    step(OldBoardT, NewBoardT),
    transpose(NewBoardT, NewBoard).
